...
def test_merge_and_push_security_manifest(
    ...
    mock_get_attestation.assert_called_once_with(
        "quay.io/org/repo@abcdef", "/temp/temp_path/attestation_abcd.json"
    )
    ...
    mock_attest.assert_called_once_with("/path/to/final/manifest.json", "quay.io/org/repo@abcdef")
...
def test_merge_and_push_security_manifest_no_product_no_existing_attestation(
    ...
    mock_add_products.assert_not_called()
    mock_delete_attestation.assert_not_called()
    mock_attest.assert_called_once_with("path/to/manifest", "quay.io/org/repo@abcdef")
...
@mock.patch("uuid.uuid4")
@mock.patch("pubtools._quay.security_manifest_pusher.QuayClient")
def test_get_source_item_security_manifest(
    mock_quay_client,
    mock_uuid,
    ...
    res = pusher.get_source_item_security_manifest(container_source_push_item, "/temp")
...
@mock.patch("uuid.uuid4")
@mock.patch("pubtools._quay.security_manifest_pusher.QuayClient")
def test_get_source_item_security_manifest_no_manifest(
    mock_quay_client,
    mock_uuid,
    mock_get_security_manifest,
    target_settings,
...
@mock.patch(
    "pubtools._quay.security_manifest_pusher.SecurityManifestPusher."
    "get_source_item_security_manifest"
)
@mock.patch(
    "pubtools._quay.security_manifest_pusher.SecurityManifestPusher."
...
def test_push_item_security_manifest_source_image(
    ...
    mock_quay_client.return_value.get_manifest.side_effect = ValueError("no manifest")
...
def test_push_security_manifests(
    ...
    mock_local_executor.return_value.__enter__.return_value.skopeo_login.assert_called_once_with(
        "quay.io", "dest-quay-user", "dest-quay-pass", authfile="/.docker/config.json"
    )
