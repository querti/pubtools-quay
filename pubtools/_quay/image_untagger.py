...
class ImageUntagger:
    ...
    def get_repo_cosign_images(self, repo_images: list[str], repo_tags: list[str]) -> set[str]:
        """
        ...
        with tempfile.TemporaryDirectory(prefix="security_manifest_") as tmp_dir:
            for image in repo_images:
                for image_type in SecurityManifestPusher.COSIGN_TRIANGULATE_TYPES:
        ...
        for repo in repo_tag_mapping:
            ...
            if len(lost_digests) > 0:
                ...
                repo_lost_cosign_imgs |= self.get_repo_cosign_images(
                    cosign_imgs_by_digest, repo_tags
                )
